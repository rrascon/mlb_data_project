...
def plot_strike_zone(data: pd.DataFrame, title: str = '', colorby: str = 'pitch_type', legend_title: str = '',
                     annotation: str = 'pitch_type', axis: Optional[axes.Axes] = None):
    """
...
def plot_teams(data: pd.DataFrame, x_axis: str, y_axis: str, title: Optional[str] = None):
    ...
    for index, row in data.iterrows():
        ...
        try:
            ...
            path = Path(CUR_PATH, 'images/logos', f"{row['Team']}.png")

            # Read in the image from the folder location
            img = OffsetImage(plt.imread(path, format="png"), zoom=.2)
